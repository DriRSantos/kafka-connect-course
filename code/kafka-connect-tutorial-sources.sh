...
###############
# C) TwitterSourceConnector in distributed mode (USED WikimediaSourceConnector instead):
# create the topic we're going to write to (created wikimedia.recent.change)
# docker run --rm -it --net=host landoop/fast-data-dev:cp3.3.0 bash
...
kafka-topics --create --topic wikimedia.recentchange.connect --partitions 3 --replication-factor 1 --bootstrap-server 127.0.0.1:9092

# Start a console consumer on that topic
kafka-console-consumer --topic demo-3-twitter --bootstrap-server 127.0.0.1:9092
kafka-console-consumer --topic wikimedia.recentchange.connect --bootstrap-server 127.0.0.1:9092
